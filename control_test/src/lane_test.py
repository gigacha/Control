...
def serWrite(speed, steering, cnt, cur_mode, cur_speed):
    ...
    if cur_mode ==2:
        ...
        # if cur_speed%2==1:
        #     cur_speed -= 1
        break_val = 155 - int(0.5*cur_speed)
        if break_val <= 0:
            break_val = 1
...
# msg 수신시 호출되는 함수
def control_cmd(steering, speed , cur_mode):
    print("======== control_cmd =========")
    cnt=0x00;
    #print('a')
    ...
    if len(result) > 17:
        ...
        print(cnt)
        cur_speed = result[6]
        serWrite(int(speed*10), int(steering*71), cnt, cur_mode, cur_speed)
    # cnt가 10일때 패킷이 잘려서 2번에 걸쳐 들어옴.+++++++++++++++++++++++++++++ 0x0a값이 아스키코드 LF(new line)!!!
        ...
        print(cnt)        
        cur_speed = result[6]
        serWrite(int(speed*10), int(steering*71), 10, cur_mode, cur_speed)
