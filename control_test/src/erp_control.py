...
def serWrite(speed, steering, cnt, cur_mode, time_interval, cur_x, cur_y):
    global i
    cur_mode = 2
    input_speed = speed
    break_val = 0x01
    ...
    i += 1
    tmp.append(i)
    _,cte = calc_nearest_index(cur_x, cur_y)
    cte_list.append(cte)
    cur_speed_list.append(speed)
...
def getOdoMsg(msg):
    ...
    if len(result) > 17:
        ...
        serWrite(int(speed_lim*10), int(steering*71), cnt, cur_mode, time_interval, cur_x, cur_y)
        start_time = time.time()
    # cnt가 10일때 패킷이 잘려서 2번에 걸쳐 들어옴.+++++++++++++++++++++++++++++ 0x0a값이 아스키코드 LF(new line)!!!
        ...
        serWrite(int(speed_lim*10), int(steering*71), 10, cur_mode, time_interval, cur_x, cur_y)
        start_time = time.time()
...
def calc_nearest_index(cur_x, cur_y):
    ...
    angle = pi_2_pi(path_yaw[ind] - atan2(dyl, dxl))
    if angle < 0:
        mind *= -1
